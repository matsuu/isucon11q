...
----------------------------------------------------------------------
    ...
    xs-src/ppport.h -- Perl/Pollution/Portability Version 3.62
    ...
    Use 'perldoc xs-src/ppport.h' to view the documentation below.
...
=head1 SYNOPSIS
  ...
  perl xs-src/ppport.h [options] [source files]
...
This version of F<xs-src/ppport.h> is designed to support operation with Perl
...
Display the version of F<xs-src/ppport.h>.
...
Tell F<xs-src/ppport.h> to check for compatibility with the given
Perl version. The default is to check for compatibility with Perl
version 5.003_07. You can use this option to reduce the output
of F<xs-src/ppport.h> if you intend to be backward compatible only
down to a certain Perl version.

=head2 --cplusplus

Usually, F<xs-src/ppport.h> will detect C++ style comments and
replace them with C style comments for portability reasons.
Using this option instructs F<xs-src/ppport.h> to leave C++
comments untouched.
...
Strip all script and documentation functionality from F<xs-src/ppport.h>.
This reduces the size of F<xs-src/ppport.h> dramatically and may be useful
if you want to include F<xs-src/ppport.h> in smaller modules without
increasing their distribution size too much.

The stripped F<xs-src/ppport.h> will have a C<--unstrip> option that allows
...
F<xs-src/ppport.h>. Also lists if it must be explicitly requested,
if it has dependencies, and if there are hints or warnings for it.

=head2 --list-unsupported

Lists the API elements that are known not to be FULLY supported by F<xs-src/ppport.h>,
...
version of Perl that F<xs-src/ppport.h> supports at all.
...
earlier Perl releases. Use
    ...
    perl xs-src/ppport.h --list-provided

to see which API elements are provided by xs-src/ppport.h.
...
also deprecated. You can safely use the supported API, as F<xs-src/ppport.h>
will provide wrappers for older Perl versions.

=item *

Although the purpose of F<xs-src/ppport.h> is to keep you from having to concern
...
more C<#define>s in your source code before the inclusion of F<xs-src/ppport.h>.
...
macro. Just C<#define> the macro before including C<xs-src/ppport.h>:

    #define DPPP_NAMESPACE MyOwnNamespace_
    #include "xs-src/ppport.h"
...
F<xs-src/ppport.h> on your source code. See the next section for
details.

=head1 EXAMPLES

To verify whether F<xs-src/ppport.h> is needed for your module, whether you
should make any changes to your code, and whether any special defines
should be used, F<xs-src/ppport.h> can be run as a Perl script to check your
source code. Simply say:

    perl xs-src/ppport.h
...
use something like this:
    ...
    perl xs-src/ppport.h --compat-version=5.6.0 --cplusplus --patch=test.diff

If you only want your code to be scanned without any suggestions
for changes, use:

    perl xs-src/ppport.h --nochanges

You can specify a different C<diff> program or options, using
the C<--diff> option:

    perl xs-src/ppport.h --diff='diff -C 10'

This would output context diffs with 10 lines of context.

If you want to create patched copies of your files instead, use:

    perl xs-src/ppport.h --copy=.new

To display portability information for the C<newSVpvn> function,
use:

    perl xs-src/ppport.h --api-info=newSVpvn

Since the argument to C<--api-info> can be a regular expression,
you can use

    perl xs-src/ppport.h --api-info=/_nomg$/

to display portability information for all C<_nomg> functions or

    perl xs-src/ppport.h --api-info=/./
...
If this version of F<xs-src/ppport.h> is causing failure during
the compilation of this module, please check if newer versions
of either this module or C<Devel::PPPort> are available on CPAN
before sending a bug report.

If F<xs-src/ppport.h> was generated using the latest version of
...
# These are tools that must be included in xs-src/ppport.h.  It doesn't work if given
# a .pl suffix.
#
 ...
 * 5.005 perls, you can use aTHXR (supplied by xs-src/ppport.h), which will
...
/* Hint: isPSXSPC, isPSXSPC_A, isPSXSPC_L1, isPSXSPC_utf8_safe
    ...
    xs-src/ppport.h SPACE version does match VT in all perl releases.  Since VT's are
    extremely rarely found in real-life files, this difference effectively
    doesn't matter */

/* Hint: isSPACE, isSPACE_A, isSPACE_L1, isSPACE_utf8_safe
    Until Perl 5.18, this did not match the vertical tab (VT).  The xs-src/ppport.h
...
/* Hint: SvPVbyte
 * Does not work in perl-5.6.1, xs-src/ppport.h implements a version
